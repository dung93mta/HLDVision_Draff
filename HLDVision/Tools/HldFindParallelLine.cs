...
{
    [Serializable]
    public class HldFindParallelLine : HldToolBase
    {
        public HldFindParallelLine()
        {
            score = 0;
        ...
        public HldFindParallelLine(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
            lineCaliper.NumberOfCaliperChanged += lineCaliper_NumberOfCaliperChanged;
        }
        ...
        {
            get { return mDistanceFromRef; }
            set { mDistanceFromRef = value; }
        }
        double mDistanceFromRef;

        FindLinefromRefLineOption mParallelOption;
        public FindLinefromRefLineOption ParallelOption { get { return mParallelOption; } set { mParallelOption = value; } }

        [InputParam]
        public HldLine RefLine
        {
            get { return mRefLine; }
            set { mRefLine = value; }
        }
        ...
        public void DrawLine(Display.HldDisplayView display)
        {
            ...
            {
                p = Pens.Yellow;
                display.DrawLine(p, RefLine.SP, RefLine.EP);
            }
        ...
        public override void Run(bool isEditMode = false)
        {
            ...
            Point3d findlinefromrefline;
            List<Point2d> ppoints = new List<Point2d>();
            List<int> exceptIndex;

            for (int i = 0; i < calBestLine.caliperPoints.Count; i++)
                ppoints.Add(new Point2d(calBestLine.caliperPoints[i].X, calBestLine.caliperPoints[i].Y));

            if (RefLine != null)
            {
                bool b = HldFunc.FindLinefromRefLine(out findlinefromrefline, out exceptIndex, RefLine.SP.X, RefLine.SP.Y, RefLine.ThetaRad, ppoints, (double)percentageOfIgnore / 100, ParallelOption, mDistanceFromRef);

                if (b)
                {
                    Point2d cp = new Point2d(findlinefromrefline.X, findlinefromrefline.Y);
                    Point2d sp = cp - new Point2d(InputImage.Width * Math.Cos(findlinefromrefline.Z), InputImage.Width * Math.Sin(findlinefromrefline.Z));
                    Point2d ep = cp + new Point2d(InputImage.Width * Math.Cos(findlinefromrefline.Z), InputImage.Width * Math.Sin(findlinefromrefline.Z));

                    Line = new HldLine(sp, ep);
                    calBestLine.bestExceptPoint = exceptIndex;
                    lastRunSuccess = true;
                }
                else
                    lastRunSuccess = false;
