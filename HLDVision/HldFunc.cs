...
namespace HLDVision
{
    ...
    public static class HldFunc
    {
        static public Point2d ImageToFixture2D(Point2d point, Mat mat)
        {
            if (mat == null) return point;

            Point2d transPt = new Point2d();
            Mat ptMat = new Mat(3, 1, MatType.CV_32FC1);
            ptMat.Set<float>(0, 0, (float)point.X);
            ptMat.Set<float>(1, 0, (float)point.Y);
            ptMat.Set<float>(2, 0, 1);
            
            Mat transPtMat = mat * ptMat;

            transPt.X = (double)transPtMat.Get<float>(0, 0) / (double)transPtMat.Get<float>(2, 0);
            transPt.Y = (double)transPtMat.Get<float>(1, 0) / (double)transPtMat.Get<float>(2, 0);
            return transPt;
        }
        ...
        /// P = P0 + k(P1 - P0)
        /// P = P2 + l(P3 - P2)
        /// P2 - P0 = k(P1 - P0) - l(P3 - P2)
        /// P2 - P0 = [(P1 - P0) | (P3 - P2)] * Traspose(k, l)
        /// <returns></returns>
        static public Point FindCrossPoint(Point P0, Point P1, Point P2, Point P3)
        {
            CvMat A = new CvMat(2, 2, MatrixType.F64C1, new double[] { (P1 - P0).X, (P2 - P3).X, (P1 - P0).Y, (P2 - P3).Y });
            CvMat B = new CvMat(2, 1, MatrixType.F64C1, new double[] { (P2 - P0).X, (P2 - P0).Y });
            A.Invert(A);
            Cv.MatMul(A, B, B);
            Point result = (P1 - P0) * B[0] + P0;
            return result;
        }

        static public Point2d FindCrossPoint(HldLine vL0, HldLine vL1)
        {
            Point2d P0 = vL0.SP; Point2d P1 = vL0.EP;
            Point2d P2 = vL1.SP; Point2d P3 = vL1.EP;
            CvMat A = new CvMat(2, 2, MatrixType.F64C1, new double[] { (P1 - P0).X, (P2 - P3).X, (P1 - P0).Y, (P2 - P3).Y });
            CvMat B = new CvMat(2, 1, MatrixType.F64C1, new double[] { (P2 - P0).X, (P2 - P0).Y });
            A.Invert(A);
            Cv.MatMul(A, B, B);
            Point2d result = (P1 - P0) * B[0] + P0;
            return result;
        }
        ...
        {
            return (Point.DotProduct(VectorP1 - VectorP0, P - VectorP0) / Getlength(VectorP1 - VectorP0));
        }
        ...
        {
            return Math.Abs(Point.CrossProduct(P1 - P0, P2 - P0) / Math.Max(Point.Distance(P0, P1), Point.Distance(P0, P2)));
        }
        ...
        static public void CopyROI(Mat src, Mat result, Mat desROI, Rect ROI)
        {
            if (desROI.Width != ROI.Width || desROI.Height != ROI.Height) return;
            CvRect cvrect = new CvRect(ROI.X, ROI.Y, ROI.Width, ROI.Height);
            IplImage ipl = src.ToIplImage();
            IplImage iplROI = desROI.ToIplImage();
            ipl.SetROI(cvrect);
            iplROI.Copy(ipl);
            ipl.ResetROI();
            result = new Mat(ipl).Clone();
            ipl.Dispose();
            iplROI.Dispose();
        }
        ...
        static public Mat WarpImage(Mat src, Mat warp)
        {
            ...
            Size dsize = new Size(Cv.Round(xmax - xmin + 1), Cv.Round(ymax - ymin + 1));
            Mat dst = new Mat(dsize, src.Type());
            Cv2.WarpPerspective(src, dst, warp, dsize, Interpolation.Linear);
        ...
        static public Point3d WarpPoint(Point3d pt, Mat warp)
        {
            ...
            double theta = pt.Z;
            Point2d x0 = WarpPoint(HldFunc.Rotate(new Point2d(pt.X + 100, pt.Y), pt.Z), warp);
        ...
        static public Point3d CalMomentResult(Point[] Contour)
        {
            ...
            Cv2.DrawContours(MomentMat, new Point[][] { resultPoints }, 0, new Scalar(255), -1, LineType.AntiAlias);
        ...
        static public Rectf Rect2Rectf(Rect rect)
        {
            return new Rectf(rect.X, rect.Y, rect.Width, rect.Height);
        }

        static public Rect Rectf2Rect(Rectf rectf)
        {
            return new Rect(Cv.Round(rectf.X), Cv.Round(rectf.Y), Cv.Round(rectf.Width), Cv.Round(rectf.Height));
        }

        static public Rect GetRect(Rect Image, Rect rect)
        {
            Rectf newRectf = GetRect(Rect2Rectf(Image), Rect2Rectf(rect));
            return Rectf2Rect(newRectf);
        }

        static public Rectf GetRect(Rectf Image, Rectf rect)
        {
            Rectf newRect = rect;
            if (rect.X < 0)
            {
        ...
        static public Rectf GetRect(Mat Image, Rectf rect)
        {
            if (Image == null)
                return rect;
            Rectf ImageRect = new Rectf(0, 0, Image.Width, Image.Height);
            return GetRect(ImageRect, rect);
        }

        static public Rect ResizeRect(Rect rect, double scale)
        {
            return new Rect(Cv.Round(rect.X * scale), Cv.Round(rect.Y * scale), Cv.Round(rect.Width * scale), Cv.Round(rect.Height * scale));
        }

        static public Size ResizeSize(Size rect, double scale)
        {
            return new Size(Cv.Round(rect.Width * scale), Cv.Round(rect.Height * scale));
        }
        ...
        public static dynamic CopyTool(object inClass, Type inType)
        {
            dynamic outTool = Activator.CreateInstance(inType) as dynamic;
            dynamic temp;

            HldSerializer serializer = new HldSerializer();
            string path = "C:\\HLD Vision\\temp.dat";

            try
            {
                serializer.SaveTool(inClass as HldToolBase, path);
                temp = serializer.Deserializing(path);

                System.Reflection.FieldInfo[] originFields = inType.GetFields(System.Reflection.BindingFlags.Instance |
                ...
                foreach (System.Reflection.FieldInfo info in originFields)
                {
                    ...
                    info.SetValue(outTool, info.GetValue(temp));
                }
        ...
        static public bool IsPossibleImage(HldImage img)
        {
