...
{
    public partial class HldBarcodeEdit : HldToolEditBase
    {
        public HldBarcodeEdit()
        {
            InitializeComponent();
            InitBinding();
        ...
        public HldBarcode Subject
        {
            get { return base.GetSubject() as HldBarcode; }
        ...
        void InitBinding()
        {
            source = new BindingSource(typeof(HldBarcode), null);

            cb_BarcodeType.DisplayMember = "Key"; cb_BarcodeType.ValueMember = "Value";
            cb_BarcodeType.DataSource = Enum.GetValues(typeof(HldBarcode.BarcodeType)).Cast<HldBarcode.BarcodeType>().ToDictionary(obj => obj.ToString(), obj => obj).ToList();
            cb_BarcodeType.DataBindings.Add("SelectedValue", source, "Type", true, DataSourceUpdateMode.OnPropertyChanged);
            ...
            nud_BarcodeIndex.DataBindings.Add("Value", source, "Index", true, DataSourceUpdateMode.OnPropertyChanged);
        ...
        #endregion
        public List<HldBarcodeResults> BarcodeResult
        {
            ...
            set
            {
                ...
                {
                    dg_Barcode_Result.DataSource = null;
                    dg_Barcode_Result.DataSource = value;

                    for (int i = 1; i < dg_Barcode_Result.Columns.Count; i++)
                    {
                        dg_Barcode_Result.Columns[i].ReadOnly = true;
                        dg_Barcode_Result.Columns[i].Resizable = DataGridViewTriState.False;
                        dg_Barcode_Result.Columns[i].Width = 40;
                    }

                    dg_Barcode_Result.Columns[0].Width = 30;
                    dg_Barcode_Result.Columns[1].Width = 80;
                    dg_Barcode_Result.Columns[2].Width = 200;
                }));
            }
        }
    }
